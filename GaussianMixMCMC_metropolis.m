% real_seismic:实际地震数据（已处理过），theta（地震角度参数），sinal2noise：信噪比参数，wavelet：地震子波矩阵，prior：先验地质参数，n_it：迭代次数，prob_map：每个位置属于不同岩性的概率，P：马尔可夫转移矩阵，facies_initial：初始岩性分布
% 返回inversion结构体，存储了反演结果（包含岩性、速度、密度等参数的采样和统计信息）
...
n_angles = length(theta);  % 角度数
I = size(real_seismic,1);  % 道数
n_facies = length(PRIOR);  % 岩性类别数

real_seismic  = real_seismic(1:end-1,:);

% 噪声协方差和数据处理。传参：先验，先验，地震子波矩阵，倾角
...
C_d = diag(var_d(:));   % 构建地震数据的协方差矩阵C_d

I_d = size(d,1);

%% INITIAL CONFIGURATION - RANDOM MODEL  随机初始化岩性分布
% 如果没有传入初始岩性分布，就用马尔科夫链随机生成一个
if nargin < 9
...
for it = 2:n_it
    ...
    %% From the previous facies configuration, drawn a facies samples from the prior 随机选一个窗口，改变一部分岩性分布
    ...
    %% Construct prior mean an covariance of the the prior p(m|pi) 计算新方案的概率（似然函数）
    if length(fieldnames(PRIOR))==2
    ...
    %% Metropolis Acceptance rule MH接受准则
...
% 统计和输出结果
% 烧入期之后的采样用于统计均值、方差、概率分布等，最后将所有结果组织成结构体输出
...
% 根据岩性分布构建物理参数的先验均值mu和协方差矩阵C，用于反演中的参数采样。 传参：先验（各物理参数的均值和协方差），当前岩性分布，空间相关性参数(控制地质参数的空间平滑性eg：相关距离)
...
% 根据岩性分布和地质趋势数据，构建岩性驱动的先验均值向量mu和协方差矩阵C。
% 传参：先验信息（包括C），各岩性对应的纵波速度趋势，横波趋势，密度趋势，当前岩性分布，空间相关性参数
