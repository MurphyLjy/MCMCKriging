addpath('.\functions\')
load('.\data\data_synth_3layers_oil_water.mat')  % 合成测试数据
load('.\data\cmaps.mat')  

% Number of iterations 迭代次数
n_it = 5000;

% Trace to be inverted from the 2D model 选定地震道
trace = 50;

%% Input data
real_seismic = real_seismic_aki; 

SNR = 10;  % 信噪比，数值越大信号越清晰
% 给地震数据添加噪声。只取第trace条道的数据
...
prob_map = ones(I,1,length(PRIOR_elasticLog))/length(PRIOR_elasticLog); %各采样点不同岩性的概率。ones：创建所有元素都为1的数组
...
% Transition matrix 转移概率矩阵。 不同岩性之间相互转移的概率
P = [0.90    0.055    0.035;
    0.20    0.8    0;
    0.2    0.2    0.60];
...
% MCMC反演主函数调用。 传参：正演得到的合成地震数据，倾角，信噪比，地震子波矩阵，先验分布，迭代次数，各采样点不同岩性的概率，转移概率矩阵
...
% 多参数综合分析
% visualize_elastic_params(INVERSION, {'Vp','Vs','Rho'}, 'pdf');
